...
def _insert_task(
    ...
    now = datetime.now(timezone.utc).isoformat()
    conn.execute(
        """INSERT INTO tasks (id, project_id, parent_task_id, title, description, phase, status, created_at, updated_at)
           VALUES (?, ?, NULL, ?, '', ?, 'backlog', ?, ?)""",
        (task_id, project_id, title, phase, now, now),
    )
    conn.commit()
...
class TestConnectionManager:
    ...
    async def test_broadcast_sends_to_all(self) -> None:
        ...
        msg = {"type": "task_updated", "payload": {"task_id": "123"}}
...
class TestEventHelpers:
    ...
    def test_enrich_task_created_event(self, conn: sqlite3.Connection) -> None:
        ...
        _insert_task(conn, task_id, project_id, title="Enriched Task")
    ...
    def test_enrich_task_updated_event(self, conn: sqlite3.Connection) -> None:
        project_id = "proj-002"
        task_id = "task-002"
        _insert_project(conn, project_id)
        _insert_task(conn, task_id, project_id, title="Updated Task")

        event = {"type": "task_updated", "payload": {"task_id": task_id}}
        enriched = enrich_event_payload(conn, event)

        assert enriched["type"] == "task_updated"
        assert enriched["payload"]["id"] == task_id
    ...
    def test_enrich_comment_added_event(self, conn: sqlite3.Connection) -> None:
        ...
        _insert_project(conn, project_id)
        _insert_task(conn, task_id, project_id)
...
class TestEventBroadcasting:
    ...
    async def test_create_task_emits_event(
        ...
        await client.post(
            ...
            json={"title": "New Task", "phase": "coder"},
        )
    ...
    @pytest.mark.asyncio
    async def test_update_task_status_emits_event(
        self, client: AsyncClient, conn: sqlite3.Connection
    ) -> None:
        proj = await client.post("/projects", json={"title": "Update Events"})
